...
def processing_data(data):
    ...
    for record in data:
        ...
        if (prize_key not in old_data[case_short_url]["keys"]):
            old_data[case_short_url]["prizes"][prize_name]["count"] += 1
            old_data[case_short_url]["keys"].append(prize_key)

    with open("prepared_data.json", 'w', encoding="utf-8") as file_output:
        json.dump(old_data, file_output, ensure_ascii=False, indent=4)
